import Link from "next/link"

const COMPANY_LINKS = [
  { href: "/bolag", label: "Våra bolag" },
  { href: "/manifest", label: "Manifest" },
  { href: "/karriar", label: "Karriär" },
  { href: "/kontakt", label: "Kontakt" },
]

const LEGAL_LINKS = [
  { href: "/integritetspolicy", label: "Integritetspolicy" },
  { href: "/anvandarvillkor", label: "Användarvillkor" },
]

export function Footer() {
  return (
    <footer className="border-t border-neutral-200 dark:border-white/[0.06] bg-neutral-50 dark:bg-neutral-950">
      <div className="max-w-6xl mx-auto px-6 py-16">
        <div className="grid gap-12 md:grid-cols-[2fr_1fr_1fr_1fr]">

          {/* Brand column */}
          <div className="space-y-4">
            <Link href="/" className="text-xl font-bold tracking-tight text-neutral-900 dark:text-white">
              Noderum
            </Link>
            <p className="text-sm text-neutral-500 dark:text-white/40 leading-relaxed max-w-xs">
              Vi grundar och driver operativa AI-bolag för svenska SMB. Tre bolag i pilotdrift. Fler på väg.
            </p>
            {/* Social */}
            <div className="flex items-center gap-3 pt-2">
              <a
                href="https://linkedin.com/company/noderum"
                target="_blank"
                rel="noopener noreferrer"
                className="w-9 h-9 rounded-lg border border-neutral-200 dark:border-white/10 flex items-center justify-center text-neutral-400 dark:text-white/30 hover:text-neutral-700 dark:hover:text-white/70 hover:border-neutral-300 dark:hover:border-white/20 transition-colors"
                aria-label="LinkedIn"
              >
                <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                </svg>
              </a>
              <a
                href="https://instagram.com/noderum"
                target="_blank"
                rel="noopener noreferrer"
                className="w-9 h-9 rounded-lg border border-neutral-200 dark:border-white/10 flex items-center justify-center text-neutral-400 dark:text-white/30 hover:text-neutral-700 dark:hover:text-white/70 hover:border-neutral-300 dark:hover:border-white/20 transition-colors"
                aria-label="Instagram"
              >
                <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z" />
                </svg>
              </a>
            </div>
          </div>

          {/* Company links */}
          <div>
            <p className="text-[10px] font-semibold tracking-[0.18em] uppercase text-neutral-400 dark:text-white/30 mb-4">
              Företaget
            </p>
            {COMPANY_LINKS.map((l) => (
              <Link
                key={l.href}
                href={l.href}
                className="block text-sm text-neutral-500 dark:text-white/40 hover:text-neutral-900 dark:hover:text-white transition-colors mb-2.5"
              >
                {l.label}
              </Link>
            ))}
          </div>

          {/* Legal links */}
          <div>
            <p className="text-[10px] font-semibold tracking-[0.18em] uppercase text-neutral-400 dark:text-white/30 mb-4">
              Juridik
            </p>
            {LEGAL_LINKS.map((l) => (
              <Link
                key={l.href}
                href={l.href}
                className="block text-sm text-neutral-500 dark:text-white/40 hover:text-neutral-900 dark:hover:text-white transition-colors mb-2.5"
              >
                {l.label}
              </Link>
            ))}
          </div>

          {/* Contact */}
          <div>
            <p className="text-[10px] font-semibold tracking-[0.18em] uppercase text-neutral-400 dark:text-white/30 mb-4">
              Kontakt
            </p>
            <a
              href="mailto:youcef@noderum.se"
              className="block text-sm text-neutral-500 dark:text-white/40 hover:text-orange-500 transition-colors mb-2.5"
            >
              youcef@noderum.se
            </a>
            <p className="text-sm text-neutral-400 dark:text-white/30">
              Stockholm, Sverige
            </p>
          </div>
        </div>
      </div>

      {/* Bottom bar */}
      <div className="border-t border-neutral-200 dark:border-white/[0.04]">
        <div className="max-w-6xl mx-auto px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-2">
          <p className="text-xs text-neutral-400 dark:text-white/25">
            © 2026 Noderum AB. Alla rättigheter förbehållna.
          </p>
        </div>
      </div>
    </footer>
  )
}
