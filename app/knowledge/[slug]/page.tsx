...
const ARTICLES: Record<string, { title: string; date: string; tag: string; body: string[] }> = {
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: `${a.title} — Noderum`, description: a.body[0] }
}
    ...
    <div className="bg-[#EFEFEF] min-h-screen pt-28 md:pt-36">
      <article className="max-w-[1440px] mx-auto px-5 sm:px-8 md:px-12 pb-24">
        ...
        <div className="max-w-[680px]">
          {a.body.map((p, i) => (
            <p key={i} className="text-[15px] sm:text-[16px] text-gray-600 leading-relaxed mb-6">{p}</p>
          ))}
